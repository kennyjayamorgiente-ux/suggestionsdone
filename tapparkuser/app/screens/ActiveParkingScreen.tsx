    ...
    try {
      ...
      while ((dataMatch = dataRegex.exec(svgString)) !== null) {
        ...
        if (dataSlotMatch && dataSlotIdMatch) {
          ...
          // Use dynamic coordinate validation based on map configuration
          ...
          const { minX, minY, maxX, maxY } = mapConfig.coordinateBounds;
          const isInRoadArea = (x < minX || y < minY || x > maxX || y > maxY);
          
          if (isInRoadArea) {
            console.log(`🚫 Skipping FU Main spot in road area: ${slotId} at (${x}, ${y}) - bounds: (${minX},${minY}) to (${maxX},${maxY})`);
            continue;
          }
      ...
      while ((match = textRegex.exec(svgString)) !== null) {
        ...
        // SPOT 4 DEBUG: Log all text elements found
        console.log(`🔍 TEXT DEBUG: Found text element: "${textContent}"`);
        console.log(`🔍 TEXT DEBUG: Full text element: ${textElement.substring(0, 200)}...`);
        
        // Check if text looks like a spot number
        if (/^\d+$/.test(textContent)) {
          console.log(`🚨 SPOT 4 DEBUG: Found numeric text: "${textContent}" - this could become spot ${textContent}!`);
          ...
          if (xMatch && yMatch) {
            ...
            console.log(`🚨 SPOT 4 DEBUG: Text coordinates: x=${x}, y=${y}`);
            console.log(`🚨 SPOT 4 DEBUG: This text could create a touchable spot at road position!`);
            
            // Check if these coordinates look like they're in a road area - use dynamic configuration
            const parkingAreaName = bookingData?.parkingArea?.name || 'FPA';
            const parkingAreaId = bookingData?.parkingArea?.id || 2;
            const mapConfig = getMapConfig(parkingAreaId, parkingAreaName);
            const { minX, minY, maxX, maxY } = mapConfig.coordinateBounds;
            const isInRoadArea = (x < minX || y < minY || x > maxX || y > maxY);
            if (isInRoadArea) {
              console.log(`🚨 SPOT 4 DEBUG: TEXT IS IN ROAD AREA! This is likely the problematic spot ${textContent}! Bounds: (${minX},${minY}) to (${maxX},${maxY})`);
            }
          }
        }
        ...
        if (/^\d+$/.test(textContent)) {
          ...
          if (xMatch && yMatch) {
            ...
            // CRITICAL FIX: Check if this text element is inside a parking spot group
            // If it is, we should NOT create a separate touchable spot from it
            const textStartIndex = textRegex.lastIndex - textElement.length;
            const precedingText = svgString.substring(Math.max(0, textStartIndex - 500), textStartIndex);
            
            // Check if the text is inside a parking spot group (has data-type="parking-slot")
            const isInParkingSpotGroup = precedingText.includes('data-type="parking-slot"') ||
                                       precedingText.includes('data-slot=') ||
                                       precedingText.includes('data-section=');
            
            if (isInParkingSpotGroup) {
              console.log(`🚫 TEXT FIX: Skipping text "${textContent}" - it's inside a parking spot group, should not create separate touchable`);
              continue;
            }
            ...
            if (!existingSpot) {
              console.log(`🚨 SPOT 4 DEBUG: Creating spot from text "${textContent}" at (${x}, ${y})`);
              spots.push({
                ...
                spotId: `text-spot-${textContent}`,
              });
              
              if (textContent === '4') {
                console.log(`🚨 SPOT 4 DEBUG: CREATED SPOT 4 FROM TEXT! This is the culprit!`);
              }
            }
          }
        }
  ...
  const fetchCapacitySections = async (areaId: number) => {
    try {
      ...
      if (response.success) {
        ...
        console.log('🏍️ Motorcycle capacity sections from database:', motorcycleSections);
        console.log('🏍️ Setting capacitySections state with:', motorcycleSections.length, 'sections');
  ...
  useEffect(() => {
    ...
    const pollReservationStatus = async () => {
      ...
      try {
        ...
        if (response.success) {
          const bookingData = response.data as unknown as BookingDetails;
                      ...
                      <View style={{
                        ...
                        >
                          ...
                          >
                            ...
                            // DEBUG: Log SVG dimensions and coordinate system
                            console.log('🔍 SVG Analysis:', {
                              svgContent: svgContent.substring(0, 200) + '...',
                              svgDimensions: svgDimensions,
                              containerDimensions: { width: svgDimensions.width, height: svgDimensions.height }
                            });
                            ...
                            console.log('📏 SVG Width/Height Matches:', {
                              widthMatch: svgWidthMatch,
                              heightMatch: svgHeightMatch
                            });
                            
                            // Container dimensions (what we pass to SvgXml)
                            const containerWidth = svgDimensions.width;
                            const containerHeight = svgDimensions.height;
                            
                            // Extract viewBox from SVG - this is the coordinate system
                            const viewBoxMatch = svgContent.match(/viewBox=["']([^"']+)["']/);
                            let viewBoxX = 0;
                            let viewBoxY = 0;
                            let viewBoxWidth = 1294; // Updated for FU Main
                            let viewBoxHeight = 358; // Updated for FU Main
                            if (viewBoxMatch) {
                              const parts = viewBoxMatch[1].trim().split(/[\s,]+/).filter(p => p).map(Number);
                              if (parts.length >= 4) {
                                viewBoxX = parts[0];
                            ...
                            console.log('📐 Coordinate System Analysis:', {
                              viewBoxMatch: viewBoxMatch,
                              viewBox: { x: viewBoxX, y: viewBoxY, width: viewBoxWidth, height: viewBoxHeight },
                              container: { width: containerWidth, height: containerHeight },
                              svgIntrinsic: { 
                                width: svgWidthMatch ? svgWidthMatch[1] : 'not found',
                                height: svgHeightMatch ? svgHeightMatch[1] : 'not found'
                              }
                            });
                            ...
                            console.log('🎯 Spot Coordinate Conversion:', {
                              spotId: spot.id,
                              originalCoords: { x: spot.x, y: spot.y, width: spot.width, height: spot.height },
                              viewBoxAdjusted: { x: spotXInViewBox, y: spotYInViewBox },
                            ...
                            console.log('📍 Final Position Calculation:', {
                              ...
                              centeringOffset: { x: offsetX, y: offsetY },
                            ...
                            // Final coordinates - exact fit to SVG spot with fine-tuning alignment
                            const left = finalX - 2; // Small adjustment for perfect alignment
                            const top = finalY - 2;  // Small adjustment for perfect alignment
                            const width = pixelWidth + 4;  // Slightly larger for better touch area
                            const height = pixelHeight + 4; // Slightly larger for better touch area
                            ...
                            // Debug logging for first few spots and current spot
                            if (__DEV__ && (clickableSpots.indexOf(spot) < 3 || spot.spotNumber === bookingData?.parkingSlot?.spotNumber)) {
                              console.log('🎯 FU Main Spot positioning:', {
                                spotId: spot.id,
                                spotNumber: spot.spotNumber,
                                section: (spot as any).section,
                                svgCoords: { x: spot.x, y: spot.y, width: spot.width, height: spot.height },
                                viewBox: { x: viewBoxX, y: viewBoxY, width: viewBoxWidth, height: viewBoxHeight },
                                container: { width: containerWidth, height: containerHeight },
                                rendered: { width: renderedWidth, height: renderedHeight },
                                scale: { x: scaleX, y: scaleY },
                                finalCoords: { left: finalX, top: finalY, width: pixelWidth, height: pixelHeight },
                                adjustedCoords: { left, top, width, height },
                                alignment: { offsetX: -2, offsetY: -2, sizeOffset: 4 },
                                parentScale: scale,
                                isParkingSlot: !!(spot as any).section,
                              });
                            }
                            ...
                            if (!spotStatus && spot.id) {
                              ...
                              if (__DEV__ && (clickableSpots.indexOf(spot) < 3)) {
                            ...
                            // FINAL SAFETY CHECK: Only render touchables for legitimate parking spots
                            // This prevents any remaining invalid elements from becoming touchable
                            const isLegitimateFinalSpot = 
                              spot.id && 
                              (spot.id.includes('FPA-') || spot.id.includes('S-') || spot.id.includes('spot') || spot.id.includes('parking') || spot.id.includes('section-')) &&
                              spot.width >= 20 && 
                              spot.height >= 20 &&
                              !isNaN(spot.x) && 
                              !isNaN(spot.y);
                            
                            // Debug logging for FPA spots
                            if (spot.id && spot.id.includes('FPA-')) {
                              console.log('🔍 DEBUG FPA Spot Final Check:', {
                                id: spot.id,
                                spotNumber: spot.spotNumber,
                                x: spot.x,
                                y: spot.y,
                                width: spot.width,
                                height: spot.height,
                                isLegitimateFinalSpot,
                                isActualSpot: !spot.id.includes('section-')
                              });
                            }
                            
                            // For capacity sections, allow more flexible coordinates (they can be outside main area)
                            const validCoordinates = isCapacitySection ? 
                              (spot.x >= -50 && spot.y >= -50 && spot.x <= 1500 && spot.y <= 500) : // Allow negative for sections
                              (spot.x >= 0 && spot.y >= 0 && spot.x <= 1500 && spot.y <= 500); // Strict for parking spots
                            
                            if (!validCoordinates) {
                              console.log('🚫 FINAL SAFETY CHECK: Skipping invalid coordinates:', {
                                id: spot.id,
                                x: spot.x,
                                y: spot.y,
                                isCapacitySection,
                                reason: isCapacitySection ? 'section coordinates out of extended bounds' : 'spot coordinates out of bounds'
                              });
                              return null;
                            }
                            
                            // Additional check: if it's a spot (not section), it must have a spotNumber
                            const isActualSpot = !spot.id.includes('section-');
                            if (isActualSpot && !spot.spotNumber) {
                              console.log('🚫 FINAL SAFETY CHECK: Skipping spot without spotNumber:', {
                                id: spot.id,
                                spotNumber: spot.spotNumber
                              });
                              return null;
                            ...
                            if (!isLegitimateFinalSpot) {
                              console.log('🚫 FINAL SAFETY CHECK: Skipping invalid touchable spot:', {
                                id: spot.id,
                                spotNumber: spot.spotNumber,
                                x: spot.x,
                                y: spot.y,
