...
export default function HomeScreen() {
  ...
  const handleSpotPress = (spot: any) => {
    ...
    const isFPACapacitySection = (spot.id === 'E' || spot.id === 'I' || spot.id === 'SD') && 
      selectedAreaForSpots?.name?.toLowerCase().includes('fpa');
    ...
    let message = `Spot: ${spot.spotNumber || spot.id}\n`;
    message += `Status: ${status.charAt(0).toUpperCase() + status.slice(1)}\n`;
    
    if (spotStatus?.spot_type && spotStatus.spot_type !== 'capacity_section') {
    ...
    // Add capacity info for capacity sections
    if (spotStatus?.spot_type === 'capacity_section' && spotStatus.totalCapacity) {
      message += `Capacity: ${spotStatus.availableCapacity}/${spotStatus.totalCapacity} available\n`;
      message += `Utilization: ${(spotStatus.utilizationRate || 0).toFixed(1)}%\n`;
    }
  ...
  const fetchCapacitySections = async (areaId: number) => {
    try {
      ...
      if (response.success && response.data) {
        ...
        // DEBUG: Check if any motorcycle sections have capacity_only mode
        const capacityOnlyMotorcycleSections = motorcycleSections.filter(section => 
          (section as any).section_mode === 'capacity_only'
        );
        if (capacityOnlyMotorcycleSections.length > 0) {
          console.log('🎯 FOUND CAPACITY-ONLY MOTORCYCLE SECTIONS:', capacityOnlyMotorcycleSections.length);
          capacityOnlyMotorcycleSections.forEach(section => {
            console.log('🎯 Capacity-only motorcycle section:', section.sectionName, section);
          });
        } else {
          console.log('ℹ️ No capacity-only motorcycle sections found');
          console.log('🔍 All motorcycle sections checked:', motorcycleSections.map(s => ({
            name: s.sectionName,
            section_mode: (s as any).section_mode,
